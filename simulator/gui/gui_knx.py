...
class GUIWindow(pyglet.window.Window):
    ...
    def __init__(self, config_path: str, default_config_path: str, empty_config_path: str, saved_config_path: str, room: Room=None, svshi_mode: bool=False, telegram_logging: bool=False) -> None:
        """
        ...
        self.__room_widget = gt.RoomWidget(gc.ROOM_WIDTH, gc.ROOM_LENGTH, self.__batch, group_bg=self.__background, group_mg=self.__middlebackground, label=self.room.name, label_group=self.__middleground)
        ...
        self.__available_devices = gt.AvailableDevices(self.__batch, group_dev=self.__foreground, group_box=self.__background)

        # Create the text labels and the textbox to display to the user
        self.__command_label = pyglet.text.Label('Enter your command',font_name=gc.FONT_SYSTEM_TITLE, font_size=20, bold=True,
                                    x=gc.COMMANDLABEL_POS[0], y=gc.COMMANDLABEL_POS[1],
                                    anchor_x='right', anchor_y='bottom',
                                    batch=self.__batch, group=self.__foreground)
        self.__text_box = pyglet.shapes.Rectangle(gc.TEXTBOX_POS[0], gc.TEXTBOX_POS[1], gc.WIN_WIDTH-gc.TEXTBOX_POS[0]-gc.WIN_BORDER, 40, color=(255, 255, 255),
        ...
        self.__input_label = pyglet.text.Label("",
                                    font_name=gc.FONT_USER_INPUT, font_size=15,
                                    color=(10, 10, 10, 255),
                                    x=(self.__text_box.x+10), y=(self.__text_box.y+20),
                                    anchor_x='left', anchor_y='center',
                                    batch=self.__batch, group=self.__foreground)
        self.simtime_widget = gt.SimTimeWidget(gc.SIMTIME_POS[0], gc.SIMTIME_POS[1], self.__batch, group_box=self.__background, group_label=self.__foreground)
        ...
        self.__devicelist_widget = gt.DeviceListWidget(gc.DEVICELIST_POS[0], gc.DEVICELIST_POS[1], self.__batch, group_label=self.__foreground, group_box=self.__background)
        self.__sensors_box_shape = pyglet.shapes.BorderedRectangle(gc.WIN_BORDER/2, gc.OFFSET_SENSOR_LEVELS_BOX_Y_BOTTOM, gc.SENSOR_LEVELS_BOX_WIDTH, gc.SENSOR_LEVELS_BOX_LENGTH, border=gc.BOX_BORDER,
                                    color=gc.COLOR_BOX_SENSORS, border_color=gc.COLOR_BOX_SENSORS_BORDER,
                                    batch=self.__batch, group=self.__background)
        self.__brightness_label = pyglet.text.Label("Brightness (lux):",
                                    ...
                                    x=gc.BRIGHTNESS_LABEL_POS[0], y=gc.BRIGHTNESS_LABEL_POS[1],
                                    anchor_x='left', anchor_y='bottom',
                                    batch=self.__batch, group=self.__foreground)
        self.__temperature_label = pyglet.text.Label("Temperature (°C):",
                                    font_name=gc.FONT_SYSTEM_TITLE, font_size=15,  bold=True,
                                    color=gc.COLOR_FONT_SENSORS_TITLE,
                                    x=gc.TEMPERATURE_LABEL_POS[0], y=gc.TEMPERATURE_LABEL_POS[1],
                                    anchor_x='left', anchor_y='bottom',
                                    batch=self.__batch, group=self.__foreground)
        self.__airquality_label = pyglet.text.Label("Air quality - T(°C) / CO2(ppm) / RH(%):",  #350-1,000 ppm, 
                                    font_name=gc.FONT_SYSTEM_TITLE, font_size=15,  bold=True,
                                    color=gc.COLOR_FONT_SENSORS_TITLE,
                                    x=gc.AIRSENSOR_LABEL_POS[0], y=gc.AIRSENSOR_LABEL_POS[1],
                                    anchor_x='left', anchor_y='bottom',
                                    batch=self.__batch, group=self.__foreground)
        # GUI buttons
        self.__buttons = [] # List of Button instances (PAUSE, STOP, RELOAD, SAVE, DEFAULT)
        # Reload Button to re-initialize the simulation
        self.__button_pause = gt.ButtonPause(gc.BUTTON_PAUSE_POS[0], gc.BUTTON_PAUSE_POS[1], self.__batch, gc.BUTTON_PAUSE_PATH, gc.BUTTON_PLAY_PATH, group=self.__foreground)
        self.__buttons.append(self.__button_pause)
        self.__button_stop = gt.ButtonStop(gc.BUTTON_STOP_POS[0], gc.BUTTON_STOP_POS[1], self.__batch, gc.BUTTON_STOP_PATH, group=self.__foreground)
        self.__buttons.append(self.__button_stop)
        self.__button_reload = gt.ButtonReload(gc.BUTTON_RELOAD_POS[0], gc.BUTTON_RELOAD_POS[1], self.__batch, gc.BUTTON_RELOAD_PATH, group=self.__foreground)
        self.__buttons.append(self.__button_reload)
        self.__button_save = gt.ButtonSave(gc.BUTTON_SAVE_POS[0], gc.BUTTON_SAVE_POS[1], self.__batch, gc.BUTTON_SAVE_PATH, group=self.__foreground)
        self.__buttons.append(self.__button_save)
        self.__button_default = gt.ButtonDefault(gc.BUTTON_DEFAULT_POS[0], gc.BUTTON_DEFAULT_POS[1], self.__batch, gc.BUTTON_DEFAULT_PATH, group=self.__foreground)
...
### Display methods ###
    def __display_devices_list(self, scroll: int=0) -> None:
        ...
        list_y = self.__devicelist_widget.deviceslist_title.y - gc.OFFSET_DEVICESLIST_TITLE 
        ...
        max_scroll = len(self.__room_devices) - self.__devicelist_widget.MAX_SIZE_DEVICES_LIST 
        if max_scroll > 0: 
        ...
        for room_device in self.__room_devices[self.__devices_scroll:]:
            ...
            room_device_label = pyglet.text.Label(label_text,
                                font_name=gc.FONT_SYSTEM_INFO, font_size=gc.FONT_SIZE_DEVICESLIST,
                                color=gc.COLOR_FONT_DEVICESLIST_DEVICE,
                                x=room_dev_x, y=room_dev_y,
                                anchor_x='left', anchor_y='bottom',
                                batch=self.__batch, group=self.__foreground)
            room_device_ia_label = pyglet.text.Label(ia_label,
                                font_name=gc.FONT_SYSTEM_INFO, font_size=gc.FONT_SIZE_INDIVIDUAL_ADDR,
        ...
        self.__devicelist_widget.update_box(new_length=room_devices_counter*gc.OFFSET_LIST_DEVICE)
    ...
    def __display_brightness_labels(self) -> None:
        ...
        for room_device in self.__room_devices:
            if 'bright' in room_device.label.text.lower():
                room_bright_x = bright_x + room_brightness_counter*gc.OFFSET_SENSOR_LEVELS
                room_bright_y = bright_y
                room_brightness_counter += 1
                room_brightness_label = pyglet.text.Label('bright'+room_device.label.text[-1],
                                    font_name=gc.FONT_SYSTEM_INFO, font_size=gc.FONT_SIZE_SENSOR_LABEL,
    ...
    def __display_temperature_labels(self) -> None:
        ...
        for room_device in self.__room_devices:
            if 'thermometer' in room_device.label.text.lower():
                room_temp_x = temp_x + room_temperature_counter*gc.OFFSET_SENSOR_LEVELS
                room_temp_y = temp_y
    ...
    def __display_airsensor_labels(self) -> None:
        ...
        for room_device in self.__room_devices:
            if 'airsensor' in room_device.label.text.lower():
                offset_x = gc.OFFSET_SENSOR_LEVELS if len(previous_airsensor_name) else 0
        ...
        for airsensor in airsensor_dict: # AirSensor names are the keys
            ...
            for room_airquality_label in self.__room_airquality_labels:
                if 'airsensor' in airsensor and airsensor == room_airquality_label.text:
                    ...
                    self.__room_airsensor_levels.append(room_sensor_level)
                    room_airquality_counter += 1
                elif 'humidityair' in airsensor and airsensor == room_airquality_label.text:
                    air_level_x = room_airquality_label.x + room_airquality_counter*gc.OFFSET_SENSOR_LEVELS
                    air_level_y = room_airquality_label.y - gc.OFFSET_SENSOR_TITLE
    ...
    def __display_windows(self) -> None:
        ...
        for window in self.room.windows:
            if window.device.wall in ['north', 'south']:
                window_widget = gt.WindowWidget(window.device, gc.WINDOW_HORIZONTAL_PATH, self.__batch, self.__foreground, self.__room_width_ratio, self.__room_length_ratio,
                                            self.__room_widget.origin_x, self.__room_widget.origin_y)
            elif window.device.wall in ['east', 'west']:
                window_widget = gt.WindowWidget(window.device, gc.WINDOW_VERTICAL_PATH, self.__batch, self.__foreground, self.__room_width_ratio, self.__room_length_ratio,
                                            self.__room_widget.origin_x, self.__room_widget.origin_y)
    ...
    def __switch_sprite(self) -> None:
        ...
        for room_device in self.__room_devices:
            try:
                if room_device.sprite_state != room_device.in_room_device.device.state: # GUI device sprite state is not the same as KNX device state
                ...
                if room_device.sprite_state and room_device.sprite_state_ratio != room_device.in_room_device.device.state_ratio:
                    room_device.sprite_state_ratio = room_device.in_room_device.device.state_ratio
                    new_opacity = gc.OPACITY_MIN + (gc.OPACITY_DEFAULT-gc.OPACITY_MIN)*room_device.sprite_state_ratio/100
...
### Devices and configuration file management methods ###
    ...
    def __update_device_location(self, pos_x: float, pos_y: float) -> None:
        ...
        loc_x, loc_y = gt.gui_pos_to_system_loc(pos_x, pos_y, self.__room_width_ratio, self.__room_width_ratio, self.__room_widget.origin_x, self.__room_widget.origin_y)
        self._moving_device.update_position(pos_x, pos_y, loc_x, loc_y, update_loc=True)
    ...
    def __replace_moving_device_in_room(self, x: float, y: float) -> None:
        ...
        loc_x, loc_y = gt.gui_pos_to_system_loc(new_x, new_y, self.__room_width_ratio, self.__room_width_ratio, self.__room_widget.origin_x, self.__room_widget.origin_y)
    ...
    def __update_config_ga(self, room_device: gt.DeviceWidget, group_address: str, detach: bool=False) -> int:
    ...
    def __launch_vacuum(self) -> None:
        ...
        if not hasattr(self, "vacuum_widget"):
            self.vacuum_widget = gt.VacuumWidget(gc.VACUUM_PATH, 1400, 650, self.__batch, self.__middleground)


## Public methods ##
    def initialize_system(self, save_config: bool=False, config_path: str='', system_dt: float=1):
        ...
        pyglet.clock.schedule_interval(self.room.update_world, interval=system_dt, gui_mode=True)
        ...
        for in_room_device in self.room.devices:
            ...
            if 'thermometer' in gui_device.label_name:
                ...
                img_neutral = None
            device_widget = gt.DeviceWidget(pos_x, pos_y, self.__batch, gui_device.file_ON, gui_device.file_OFF, group=self.__foreground, device_class=in_room_device.name[:-1], device_number=in_room_device.name[-1], img_neutral=img_neutral)
    ...
    def update_sensors(self, brightness_levels: List[Tuple[str, float]], temperature_levels: List[Tuple[str, float]], rising_temp: bool, humidity_levels: List[Tuple[str, float]], co2_levels: List[Tuple[str, float]], humiditysoil_levels: List[Tuple[str, float]], presence_sensors_states: List[Tuple[str, bool]]) -> None:
    ...
    def reload_simulation(self, default_config: bool=False, empty_config: bool=False) -> None:
        """ Reload the simulation with initial configuration file """
        from tools import configure_system_from_file
        pyglet.clock.unschedule(self.room.update_world)
        for room_device in self.__room_devices: # Re-Initialisation of the room devices list
            room_device.delete()
        self.__room_devices = []
        ...
        for room_brightness_label in self.__room_brightness_labels: # Re-Initialisation of the room brightness labels list
            room_brightness_label.delete()
        self.__room_brightness_labels = []
        for room_device_label in self.__room_devices_labels: # Re-Initialisation of the room device labels list
            room_device_label.delete()
...
### Mouse events ###
    def on_mouse_press(self, x, y, button, modifiers) -> None:
                    ...
                    if room_device.hit_test(x, y):
                        if isinstance(room_device.in_room_device.device, FunctionalModule):
                            if isinstance(room_device.in_room_device.device, Dimmer):
                                ...
                                self._dimmer_being_set = gt.DimmerSetterWidget(room_device)
                                return
                        ...
                        if isinstance(room_device.in_room_device.device, HumiditySoil):
                            if hasattr(room_device, "humiditysoil"):
                                room_device.humiditysoil = room_device.in_room_device.device.humiditysoil = 90 # Fully wet loam soil
                                room_device.update_drop_sprite()
            # LEFT click + CTRL : assign a group address to a device
            elif modifiers & pyglet.window.key.MOD_CTRL:
                for room_device in self.__room_devices:
                    if room_device.hit_test(x, y):
                        if self.room.attach(room_device.in_room_device.device, self.__input_label.text):
            ...
            # LEFT click + OPTION : add/remove persons 
                ...
                if child and not sitting:
                    if self.person_child.hit_test(x, y):
                        ...
                        self.person_moving = self.person_sitting = gt.PersonWidget(gc.PERSON_SITTING_PATH, x, y, self.__batch, self.__foreground)
                ...
                if sitting and not child:
                    if self.person_sitting.hit_test(x, y):
                        ...
                        self.person_moving = self.person_child = gt.PersonWidget(gc.PERSON_CHILD_PATH, x, y, self.__batch, self.__foreground)
                        ...
                        if device.hit_test(x, y): 
                            ...
                            for room_device in self.__room_devices:
                                if room_device.device_class.lower() in device_class.lower():
                                    similar_dev_counter += 1
                            self._moving_device = gt.DeviceWidget(x, y, self.__batch, device.file_ON, device.file_OFF, group=self.__foreground, device_class=device_class, device_number=str(similar_dev_counter))
                            return
                    for room_device in self.__room_devices:
                        if room_device.hit_test(x, y):
        ...
        if button == pyglet.window.mouse.RIGHT:
            ...
            # RIGHT click : Empty config from default Button
                ...
                for button in self.__buttons:
                    if button.widget.hit_test(x, y) and isinstance(button, gt.ButtonDefault):
    ...
    def on_mouse_drag(self, x, y, dx, dy, buttons, modifiers) -> None:
        ...
        if (modifiers & pyglet.window.key.MOD_SHIFT):
            if hasattr(self, '_dimmer_being_set'):
                if not self._dimmer_being_set.being_set:
                    self._dimmer_being_set.start_setting_dimmer(self.__batch, self.__foreground) 
                new_ratio = gt.dimmer_ratio_from_mouse_pos(y, self._dimmer_being_set.center_y)
