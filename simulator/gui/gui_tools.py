...
class DeviceListWidget(object):
    ...
    def __init__(self, x: float, y: float, batch: Batch, group_label: OrderedGroup, group_box: OrderedGroup) -> None:
        """
        ...
        self.MAX_SIZE_DEVICES_LIST = (y - gc.OFFSET_LIST_DEVICE) // gc.OFFSET_LIST_DEVICE
        self.__box_shape = pyglet.shapes.BorderedRectangle(gc.WIN_BORDER/2, self.__topleft_y-self.__length, gc.DEVICE_LIST_BOX_WIDTH, self.__length, border=gc.BOX_BORDER,
                                            color=gc.COLOR_BOX_DEVICESLIST, border_color=gc.COLOR_BOX_DEVICESLIST_BORDER,
                                            batch=batch, group=self.__group_box)
        self.deviceslist_title = pyglet.text.Label("Devices in the Room:",
                                    font_name=gc.FONT_SYSTEM_TITLE, font_size=15, bold=True,
    ...
    def update_box(self, new_length: int) -> None:
        ...
        self.__length = new_length + gc.OFFSET_DEVICELIST_BOX_TOP + gc.OFFSET_DEVICELIST_BOX_BOTTOM
        self.__box_shape.delete()
        self.__box_shape = pyglet.shapes.BorderedRectangle(gc.WIN_BORDER/2, self.__topleft_y-self.__length, gc.DEVICE_LIST_BOX_WIDTH, height=self.__length, border=gc.BOX_BORDER,
                                            color=gc.COLOR_BOX_DEVICESLIST, border_color=gc.COLOR_BOX_DEVICESLIST_BORDER,
                                            batch=self.__batch, group=self.__group_box)
    
    def hit_test(self, x: float, y: float) -> bool:
        """Test if DeviceList widget was hit by the mouse."""
        return (self.__box_shape.x < x < (self.__box_shape.x + gc.DEVICE_LIST_BOX_WIDTH)
...
class SimTimeWidget(object):
    ...
    def __init__(self, x: float, y: float, batch: Batch, group_box: OrderedGroup, group_label: OrderedGroup) -> None:
        """
        Initialization of pyglet Device List widget object.
        group_box should be behind group_label.
        """
        self.__box_shape = pyglet.shapes.BorderedRectangle(x, y-gc.SIMTIME_BOX_LENGTH/2, gc.SIMTIME_BOX_WIDTH, gc.SIMTIME_BOX_LENGTH, border=gc.BOX_BORDER,
                                            color=gc.COLOR_BOX_SIMTIME, border_color=gc.COLOR_BOX_SIMTIME_BORDER,
                                            batch=batch, group=group_box)
        self.__simtime_label = pyglet.text.Label("SimTime:",
                                    font_name=gc.FONT_SYSTEM_TITLE, font_size=gc.FONT_SIZE_DATETIME_TITLE, bold=True,
                                    color = gc.COLOR_FONT_SIMTIME_LABEL,
                                    x=x+gc.OFFSET_SIMTIME_BOX, y=y,
                                    anchor_x='left', anchor_y='bottom',
                                    batch=batch, group=group_label)
        self.simtime_value = pyglet.text.Label("0:00:00", 
                                    font_name=gc.FONT_SYSTEM_TITLE, font_size=gc.FONT_SIZE_SIMTIME, bold=True,
                                    color = gc.COLOR_FONT_SIMTIME_VALUE,
                                    x=x+gc.OFFSET_SIMTIME_BOX+gc.OFFSET_SIMTIME_VALUE, y=y,
                                    anchor_x='left', anchor_y='bottom',
                                    batch=batch, group=group_label)
        self.__date_label = pyglet.text.Label("Date: ",
                                    font_name=gc.FONT_SYSTEM_TITLE, font_size=gc.FONT_SIZE_DATETIME_TITLE, bold=True,
                                    color = gc.COLOR_FONT_SIMTIME_LABEL,
                                    x=x+gc.OFFSET_SIMTIME_BOX, y=y-gc.OFFSET_SIMTIME_DATE,
                                    anchor_x='left', anchor_y='bottom',
                                    batch=batch, group=group_label)
        self.date_value = pyglet.text.Label("", 
                                    font_name=gc.FONT_SYSTEM_TITLE, font_size=gc.FONT_SIZE_DATE, bold=True,
                                    color = gc.COLOR_FONT_SIMTIME_VALUE,
                                    x=x+gc.OFFSET_SIMTIME_BOX+gc.OFFSET_DATETIME_VALUE, y=y-gc.OFFSET_SIMTIME_DATE,
                                    anchor_x='left', anchor_y='bottom',
                                    batch=batch, group=group_label)

class DayTimeWeatherWidget(object):
    """ Class to represent the simulation outside weather, outside states and their GUI box"""
    def __init__(self, x: float, y: float, batch: Batch, group_box: OrderedGroup, group_daytime: OrderedGroup, group_weather: OrderedGroup, temp_out: float, hum_out: float, co2_out: float) -> None:
        """
        Initialization of pyglet Device List widget object.
        ...
        self.__box_shape = pyglet.shapes.BorderedRectangle(self.__pos_x, self.__pos_y, gc.TIMEWEATHER_BOX_WIDTH, gc.TIMEWEATHER_BOX_LENGTH, border=gc.BOX_BORDER,
                                    color=gc.COLOR_BOX_WEATHER, border_color=gc.COLOR_BOX_WEATHER_BORDER,
        ...
        self.__out_state_label = pyglet.text.Label("Out state:",
                                    font_name=gc.FONT_SYSTEM_TITLE, font_size=gc.FONT_SIZE_OUT_STATE, bold=True,
                                    color = gc.COLOR_FONT_OUTSTATE_LABEL,
                                    x=self.__pos_x+gc.OFFSET_OUTSTATE_LABEL, y=self.__pos_y+gc.OFFSET_OUTSTATE_LABEL,
                                    anchor_x='left', anchor_y='bottom',
                                    batch=self.__batch, group=self.__group_daytime)
        self.__out_state_value = pyglet.text.Label(self.__out_state_str,
                                    font_name=gc.FONT_SYSTEM_TITLE, font_size=gc.FONT_SIZE_OUT_STATE, bold=True,
                                    color = gc.COLOR_FONT_OUTSTATE_VALUE,
                                    x=self.__pos_x+gc.OFFSET_OUTSTATE_LABEL+gc.OFFSET_OUTSTATE_VALUE, y=self.__pos_y+gc.OFFSET_OUTSTATE_LABEL,
                                    anchor_x='left', anchor_y='bottom',
                                    batch=self.__batch, group=self.__group_daytime)
        ...
        self.__tod_dict= {"sunrise": {"img": self.__sunrise_img, "offset_x": gc.OFFSET_SUNRISE_X, "offset_y": gc.OFFSET_SUNRISE_SUNSET_Y}, 
                            "sun": {"img": self.__sun_img, "offset_x": gc.OFFSET_SUN_MOON_X, "offset_y": gc.OFFSET_SUN_MOON_Y},
                            "sunset": {"img": self.__sunset_img, "offset_x": gc.OFFSET_SUNSET_X, "offset_y": gc.OFFSET_SUNRISE_SUNSET_Y}, 
                            "moon": {"img": self.__moon_img, "offset_x": gc.OFFSET_SUN_MOON_X, "offset_y": gc.OFFSET_SUN_MOON_Y}}
        self.__weather_dict = {"overcast": {"img": self.__cloud_overcast_img, "offset_x_ratio": gc.OFFSET_CLOUD_WIDTH_RATIO, "offset_y_ratio": gc.OFFSET_CLOUD_LENGTH_RATIO}, 
                                "dark": {"img": self.__cloud_dark_img, "offset_x_ratio": gc.OFFSET_CLOUD_DARK_WIDTH_RATIO, "offset_y_ratio": gc.OFFSET_CLOUD_DARK_LENGTH_RATIO}}
...
class ButtonWidget(object):
    ...
    def __init__(self, x: float, y: float, batch: Batch, button_file: str, group: OrderedGroup, label_text: str='') -> None:
        ...
        self.label = pyglet.text.Label(label_text,
                                    font_name=gc.FONT_BUTTON, font_size=10,
                                    color=gc.COLOR_FONT_BUTTON,
                                    x=(self.__pos_x+self.__width//2), y=(self.__pos_y-gc.OFFSET_LABEL_BUTTON),
                                    anchor_x='center', anchor_y='center',
                                    batch=self.__batch, group=self.__group)

    def hit_test(self, x: float, y: float) -> bool:
        """Test if Button widget was hit by the mouse."""
        return (self.__pos_x < x < (self.__pos_x + self.__width) 
               and self.__pos_y < y < (self.__pos_y + self.__length))
...
class ButtonPause(object):
    ...
    def __init__(self, x: float, y: float, batch: Batch, button_pause_file: str, button_play_file: str, group: OrderedGroup) -> None:
    ...
    def activate(self, gui_window) -> None: # gui_window: gui.GUIWindow
        ...
        if gui_window.room.simulation_status: # Pause button PNG if simulation running
            self.file_to_use = self.pause_file
        elif not gui_window.room.simulation_status: # Play button PNG if simulation paused
...
class ButtonStop(object):
    ...
    def __init__(self, x: float, y: float, batch: Batch, button_file: str, group: OrderedGroup) -> None:
    ...
    def activate(self, gui_window) -> None: # gui_window: gui.GUIWindow, gui_window to be compliant with the call from gui_knx.py module
        pyglet.app.exit()
...
class DeviceWidget(object):
    ...
    def __init__(self, pos_x: float, pos_y: float, batch: Batch, img_file_ON: str, img_file_OFF: str, group: OrderedGroup, device_class: str, device_number: str, available_device: bool=False, img_neutral=None) -> None: 
        """
        ...
        if available_device:
            ...
            if "humiditysoil" in self.label_name: 
                ...
                self.__drop_sprite = pyglet.sprite.Sprite(self.__drop_red, x=self.__drop_pos_x, y=self.__drop_pos_y, batch=self.__batch, group=self.__group)
                self.__drop_label = pyglet.text.Label('10%',
                                    font_name=gc.FONT_INTERACTIVE, font_size=gc.FONT_SIZE_INTERACTIVE,
        ...
        if img_neutral is not None:
            ...
            self.sprite = pyglet.sprite.Sprite(self._img_neutral, x=self.origin_x, y=self.origin_y, batch=self.__batch, group=self.__group)
        else:
            self.sprite = pyglet.sprite.Sprite(self.img_OFF, x=self.origin_x, y=self.origin_y, batch=self.__batch, group=self.__group)
        self.label = pyglet.text.Label(self.label_name,
                                    font_name=gc.FONT_DEVICE, font_size=gc.FONT_SIZE_SOILMOISTURE,
                                    color = label_color,
                                    x=(self.origin_x+self.width//2), y=(self.origin_y-gc.OFFSET_LABEL_DEVICE),
                                    anchor_x='center', anchor_y='center',
                                    batch=self.__batch, group=self.__group)
    ...
    def hit_test(self, x, y) -> bool:
        ...
        if hasattr(self, "humiditysoil"):
            return ((self.sprite.x < x < self.sprite.x+self.width
                    and self.sprite.y < y < self.sprite.y+self.length)
                    or (self.__drop_sprite.x < x < self.__drop_sprite.x+self.__drop_sprite.width
                    and self.__drop_sprite.y < y < self.__drop_sprite.y+self.__drop_sprite.height))
        else:
            return (self.sprite.x < x < self.sprite.x+self.width
                    and self.sprite.y < y < self.sprite.y+self.length)

    def update_position(self, new_x: float, new_y: float, loc_x: float=0, loc_y: float=0, update_loc: bool=False) -> None:
        ...
        self.label.update(x=(self.origin_x+self.width//2), y=(self.origin_y-gc.OFFSET_LABEL_DEVICE))
        if "humiditysoil" in self.label_name:
            self.__drop_pos_x = self.pos_x + self.width//2
            self.__drop_pos_y = self.pos_y
            self.__drop_label_pos_x = self.__drop_pos_x
    ...
    def update_drop_sprite(self) -> None:
        ...
        if hasattr(self, 'humiditysoil'):
            ...
            self.__drop_sprite.delete()
            self.__drop_label.delete()
            self.__drop_sprite = pyglet.sprite.Sprite(drop, x=self.__drop_pos_x, y=self.__drop_pos_y, batch=self.__batch, group=self.__group)
    ...
    def update_thermometer_sprite(self, rising_temp: bool) -> None:
        ...
        if 'thermometer' in self.label_name:
            if rising_temp is None and hasattr(self, '_img_neutral'):
                ...
                self.sprite.delete()
                self.sprite = pyglet.sprite.Sprite(self.img_ON, x=self.origin_x, y=self.origin_y, batch=self.__batch, group=self.__group)
...
class AvailableDevices(object):
    ...
    def __init__(self, batch: Batch, group_dev: OrderedGroup, group_box: OrderedGroup) -> None:
        """ 
        ...
        self.__box_shape = pyglet.shapes.BorderedRectangle(gc.WIN_BORDER/2, gc.OFFSET_AVAILABLEDEVICES_LINE3-gc.OFFSET_AVAILABLE_DEVICES-gc.WIN_BORDER/2, gc.AVAILABLE_DEVICES_BOX_WIDTH, gc.AVAILABLE_DEVICES_BOX_LENGTH, border=gc.BOX_BORDER,
                                            color=gc.COLOR_BOX_AVAILABLEDEVICES, border_color=gc.COLOR_BOX_AVAILABLEDEVICES_BORDER,
                                            batch=batch, group=group_box)

        # Line 1
        self.led = DeviceWidget(gc.WIN_BORDER, gc.OFFSET_AVAILABLEDEVICES_LINE1, batch, gc.DEVICE_LED_ON_PATH, gc.DEVICE_LED_OFF_PATH, group_dev, "LED", '', available_device=True)
        self.devices.append(self.led)
        next_image_offset_x = self.led.pos_x + self.led.width + gc.OFFSET_AVAILABLE_DEVICES
        self.heater = DeviceWidget(next_image_offset_x, gc.OFFSET_AVAILABLEDEVICES_LINE1, batch, gc.DEVICE_HEATER_ON_PATH, gc.DEVICE_HEATER_OFF_PATH, group_dev, "Heater", '',  available_device=True)
        self.devices.append(self.heater)
        next_image_offset_x = self.heater.pos_x + self.heater.width + gc.OFFSET_AVAILABLE_DEVICES
        self.ac = DeviceWidget(next_image_offset_x, gc.OFFSET_AVAILABLEDEVICES_LINE1, batch, gc.DEVICE_AC_ON_PATH, gc.DEVICE_AC_OFF_PATH, group_dev, "AC", '',  available_device=True)
        self.devices.append(self.ac)
        next_image_offset_x = self.ac.pos_x + self.ac.width + gc.OFFSET_AVAILABLE_DEVICES
        self.switch = DeviceWidget(next_image_offset_x, gc.OFFSET_AVAILABLEDEVICES_LINE1, batch, gc.DEVICE_SWITCH_ON_PATH, gc.DEVICE_SWITCH_OFF_PATH, group_dev, "Switch", '', available_device=True)
        self.devices.append(self.switch)
        # next_image_offset_x = self.switch.pos_x + self.switch.width + OFFSET_AVAILABLE_DEVICES

        # Line 2
        self.button = DeviceWidget(gc.WIN_BORDER, gc.OFFSET_AVAILABLEDEVICES_LINE2, batch, gc.DEVICE_BUTTON_ON_PATH, gc.DEVICE_BUTTON_OFF_PATH, group_dev, "Button", '', available_device=True)
        self.devices.append(self.button)
        next_image_offset_x = self.button.pos_x + self.button.width + gc.OFFSET_AVAILABLE_DEVICES
        self.dimmer = DeviceWidget(next_image_offset_x, gc.OFFSET_AVAILABLEDEVICES_LINE2, batch, gc.DEVICE_DIMMER_ON_PATH, gc.DEVICE_DIMMER_OFF_PATH, group_dev, "Dimmer", '', available_device=True)
        self.devices.append(self.dimmer)
        next_image_offset_x = self.dimmer.pos_x + self.dimmer.width + 1.2*gc.OFFSET_AVAILABLE_DEVICES
        self.presencesensor = DeviceWidget(next_image_offset_x, gc.OFFSET_AVAILABLEDEVICES_LINE2, batch, gc.DEVICE_PRESENCE_ON_PATH, gc.DEVICE_PRESENCE_OFF_PATH, group_dev, "PresenceSensor", '',  available_device=True)
        self.devices.append(self.presencesensor)
        next_image_offset_x = self.presencesensor.pos_x + self.presencesensor.width + 5*gc.OFFSET_AVAILABLE_DEVICES
        self.thermometer = DeviceWidget(next_image_offset_x, gc.OFFSET_AVAILABLEDEVICES_LINE2+gc.OFFSET_AVAILABLE_DEVICES, batch, gc.DEVICE_THERMO_HOT_PATH, gc.DEVICE_THERMO_COLD_PATH, group_dev, "Thermometer", '', available_device=True, img_neutral = gc.DEVICE_THERMO_NEUTRAL_PATH)
        ...
        # Line 3
        self.brightness = DeviceWidget(gc.WIN_BORDER, gc.OFFSET_AVAILABLEDEVICES_LINE3, batch, gc.DEVICE_BRIGHT_SENSOR_PATH, gc.DEVICE_BRIGHT_SENSOR_PATH, group_dev, "Brightness", '', available_device=True)
        self.devices.append(self.brightness)
        next_image_offset_x = self.brightness.pos_x + self.brightness.width + 0.1*gc.OFFSET_AVAILABLE_DEVICES
        self.airsensor = DeviceWidget(next_image_offset_x, gc.OFFSET_AVAILABLEDEVICES_LINE3, batch, gc.DEVICE_AIRSENSOR_PATH, gc.DEVICE_AIRSENSOR_PATH, group_dev, "AirSensor", '', available_device=True)
        self.devices.append(self.airsensor)
        next_image_offset_x = self.airsensor.pos_x + self.airsensor.width + 0.4*gc.OFFSET_AVAILABLE_DEVICES
        self.co2sensor = DeviceWidget(next_image_offset_x, gc.OFFSET_AVAILABLEDEVICES_LINE3, batch, gc.DEVICE_CO2_PATH, gc.DEVICE_CO2_PATH, group_dev, "CO2Sensor", '',  available_device=True)
        self.devices.append(self.co2sensor)
        next_image_offset_x = self.co2sensor.pos_x + self.co2sensor.width + gc.OFFSET_AVAILABLE_DEVICES
        self.humidityair = DeviceWidget(next_image_offset_x, gc.OFFSET_AVAILABLEDEVICES_LINE3, batch, gc.DEVICE_HUMIDITYAIR_PATH, gc.DEVICE_HUMIDITYAIR_PATH, group_dev, "HumidityAir", '',  available_device=True)
        self.devices.append(self.humidityair)
        next_image_offset_x = self.humidityair.pos_x + self.humidityair.width + 1.85*gc.OFFSET_AVAILABLE_DEVICES
        self.humiditysoil = DeviceWidget(next_image_offset_x, gc.OFFSET_AVAILABLEDEVICES_LINE3, batch, gc.DEVICE_HUMIDITYSOIL_PATH, gc.DEVICE_HUMIDITYSOIL_PATH, group_dev, "HumiditySoil", '',  available_device=True)
...
class DimmerSetterWidget(object):
    ...
    def __init__(self, room_dimmer_widget: DeviceWidget) -> None:
        ...
        self.center_x, self.center_y = room_dimmer_widget.pos_x, room_dimmer_widget.pos_y
        self.state_label_x = self.center_x + room_dimmer_widget.width//2 + gc.OFFSET_DIMMER_RATIO
...
class RoomWidget(object):
    ...
    def __init__(self, width: float, length: float, batch: Batch, group_bg: OrderedGroup, group_mg: OrderedGroup, label_group: str, label: str) -> None:
        """
        ...
        self.__origin_x_shape = gc.WIN_WIDTH - width - gc.WIN_BORDER - 2*gc.ROOM_BORDER
        ...
        self.__batch = batch
        self.__shape = pyglet.shapes.BorderedRectangle(self.__origin_x_shape, self.__origin_y_shape, width+2*gc.ROOM_BORDER, length+2*gc.ROOM_BORDER, border=gc.ROOM_BORDER,
                                            color=gc.COLOR_ROOM, border_color=gc.COLOR_ROOM_BORDER,
                                            batch=self.__batch, group=group_bg)
        ...
        self.__label = pyglet.text.Label(self.name,
                                    font_name=gc.FONT_SYSTEM_INFO, font_size=75,
...
class PersonWidget(object):
    ...
    def __init__(self, img_path: str, x: float, y: float, batch: Batch, group: OrderedGroup):
    ...
    def hit_test(self, x: float, y: float) -> bool:
        ...
        return (self.__origin_x < x < (self.__origin_x + self.__width)
...
# Useful functions
def gui_pos_to_system_loc(pos_x: float, pos_y: float, width_ratio: float, length_ratio: float, room_x: float, room_y: float) -> Tuple[float, float]:
...
def dimmer_ratio_from_mouse_pos(mouse_y: float, dimmer_center_y: float) -> float:
    ...
    if relative_distance >= gc.OFFSET_MAX_DIMMER_RATIO:
        ...
        ratio = round(100 * (relative_distance+gc.OFFSET_MAX_DIMMER_RATIO) / (2*gc.OFFSET_MAX_DIMMER_RATIO), 2)
        return ratio