...
class KNXBus:
    ...
    def __init__(self) -> None: #, svshi_mode: bool
        ...
        __ga_buses : list of GroupAddressBus objects, containing list of devices assigned to a particular group address"""
        self.name = "KNX Bus"
        self.group_addresses = [] 
        self.__ga_buses: List[GroupAddressBus] = []

    def attach(self, device, group_address: GroupAddress) -> None: # device : Device, not InRoomDevice
        """
        Assign a device to a group address, and connect it to the bus.
        The KNX Bus object can then call the correct devices' method when a telegram arrive with corresponding destination address:
        (e.g. actuator.update_state() in transmit_telegram())
        If it is a new group address, creation of a GroupAddressBus object,
        if it is a new device on existing group address, addition of teh device to the GroupAddressBus object.
    ...
    def detach(self, device, group_address: GroupAddress) -> None: # device : Device, not InRoomDevice
        """ Remove devices from KNX Bus object, from GroupAddressBus objects, and delete group adrress from device' group addresses list."""
        if group_address not in self.group_addresses:
            logging.warning(
            ...
            for ga_bus in self.__ga_buses:
                if ga_bus.group_address == group_address:
                    if not ga_bus.detach_device(device): # return number of devices linked to this ga_bus after removal of device, if none, we delete the ga bus
    ...
    def get_info(self) -> Dict[str, Union[str, Dict[str,Dict[str, List[str]]]]]:
        """ Return information about the KNX Bus configuration, and the devices assigned to each group address, method called via CLI commmand 'getinfo'"""
...
class GroupAddressBus:
    ...
    def add_device(self, device) -> None: # device : Device, not InRoomDevice
        """ Add a device to the corresponding list (actuators, sensors or functional modules), and add the group address to the device's list of ga"""
        from devices import Actuator, Sensor, FunctionalModule
