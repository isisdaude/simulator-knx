...
class InRoomDevice:
        ...
        def get_irdev_info(self, attribute: str=None) -> Dict[str, Union[str, float, bool, Tuple[float, float, float]]]:
            """ 
            Return information about the Room Devic's' states, location and configuration, method called via CLI commmand 'getinfo'.
...
class Room:
    """Class to represent a room, containing devices at certain locations connected through a KNX Bus and a physical world representation."""
    def __init__(self, name: str, width: float, length: float, height:float, simulation_speed_factor:float, group_address_style:str, system_dt: float=1, insulation: str='average', temp_out: float=20.0, hum_out: float=50.0, co2_out: float=300, temp_in: float=25.0, hum_in: float=35.0, co2_in: float=800, date_time: str="today", weather: str="clear", test_mode: bool=False, svshi_mode: bool=False, telegram_logging: bool=False, interface: Interface=None):
        """ 
        ...
        interface : if a user reloads the simulation in the GUI, the interface that initiated the connection with svshi is used to avoid stopping the thread.
        """
    ...
    def add_device(self, device: Device, x: float, y: float, z: float=1) -> InRoomDevice:
        """ 
        ...
        from devices import FunctionalModule, Actuator, LightActuator, TemperatureActuator, Sensor, Brightness, Thermometer, AirSensor, HumiditySoil, HumidityAir, CO2Sensor, PresenceSensor
    ...
    def get_world_info(self, ambient: str=None, str_mode: bool=True) -> Dict[str, str]:
        """ Return information about the World physical states, indoor and/or outdoor, room insulation and simulation time, method called via CLI commmand 'getinfo'"""
    ...
    def get_bus_info(self) -> Dict[str, Union[str, Dict[str,Dict[str, List[str]]]]]:
        """ Return information about the bus, the group address encoding style and the devices assigned to a group address, method called via CLI commmand 'getinfo'"""
    ...
    def get_room_info(self) -> Dict[str, Union[str, List[str]]]:
        """ Return information about the room, the insulation and dimensions, and the room devices, method called via CLI commmand 'getinfo'"""
