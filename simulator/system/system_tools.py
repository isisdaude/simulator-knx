...
class IndividualAddress:
    ...
    def __init__(self, area: Union[str, int], line: Union[str, int], main: Union[str, int]) -> None:
...
class GroupAddress:
    ...
    def __init__(self, encoding_style: str, main: int, middle: int=0, sub: int=0) -> None:
        """ 
...
class Window:
    ...
    def __init__(self, window_name: str, room, wall: str, location_offset: float, size: Tuple[float, float], test_mode: bool=False) -> None: # room : Room
        ...
        necessary to scale the window if different size, e.g. if window size = 1m, scale factor x(horizontal) ou y(vertical) = 1/3.75``

        wall : 'north', 'south', 'east' or 'west', indicate where th winwod is located
        location_offset : location in meters from beginning of wall (left side of north/south walls, bottom side of east/west walls)
        ...
        self.wall, self.window_loc, self.size  = check_window(wall, location_offset, size, room)
