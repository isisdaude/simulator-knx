...
room_conf, system_dt = tools.configure_system_from_file(config_path, system_dt=1, test_mode=True)
...
# Initial state
room_conf.update_world(interval=system_dt, gui_mode=False) # first_update flag is True after system initialization, no update is done simlpy set sensors values
...
system_humiditysoil1 = room_conf.devices[room_conf.devices.index(ir_humiditysoil1)].device
system_airsensor1 = room_conf.devices[room_conf.devices.index(ir_airsensor1)].device
system_presencesensor1 = room_conf.devices[room_conf.devices.index(ir_presencesensor1)].device
...
def test_cli_commands(capfd):
    ...
    ret = tools.user_command_parser("getinfo world dummy_option", room_conf) # test if wrong option case handled
    assert ret == 0
    ...
    ret = tools.user_command_parser("getinfo room dummy_option", room_conf) # test if wrong option case handled
    assert ret == 0
    # 'getinfo bus'
    capfd.readouterr() # empty buffer
    ...
    ret = tools.user_command_parser("getinfo bus dummy_option", room_conf) # test if wrong option case handled
    assert ret == 0
...
async def test_api_commands(capfd):
    ...
    # 'set presencesensor'
    ret,_ = await parser_object.script_command_parser(room_conf, "set presencesensor1 True")
    assert ret == 1
    assert system_presencesensor1.state == True
    ret,_ = await parser_object.script_command_parser(room_conf, "set presencesensor1 False")
    assert ret == 1
    ...
    # 'set temperature'
    ret,_ = await parser_object.script_command_parser(room_conf, "set temperature 32 in")
    assert ret == 1
    assert room_conf.world.ambient_temperature._AmbientTemperature__temperature_in == 32
    assert system_airsensor1.temperature == 32
    ret,_ = await parser_object.script_command_parser(room_conf, "set temperature 30 out")
    assert ret == 1
    assert room_conf.world.ambient_temperature.temperature_out == 30
    ret,_ = await parser_object.script_command_parser(room_conf, "set temperature 24d out") # test if wrong value case handled
    assert ret is None
    # 'set humidity'
    ...
    ret,_ = await parser_object.script_command_parser(room_conf, "set humidity 24d out") # test if wrong value case handled
    assert ret is None
    # 'set co2'
    ...
    ret,_ = await parser_object.script_command_parser(room_conf, "set co2 24d out") # test if wrong value case handled
    assert ret is None
    # 'set presence'
    ...
    ret,_ = await parser_object.script_command_parser(room_conf, "set weather overcast")
    assert ret == 1
    ...
    ret,_ = await parser_object.script_command_parser(room_conf, "set weather dummy_weather")
    assert ret is None
    ret,_ = await parser_object.script_command_parser(room_conf, "set weather clear in")
    assert ret is None

    ## 'store'
    # 'store world'
    ret,_ = await parser_object.script_command_parser(room_conf, "store world simtime sim_time1")
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store world temperature temp1") # 32°
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store world humidity hum1") # 20%
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store world co2 co21") # 550 ppm
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store world brightness bright1") # 3877.0 lux
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store world weather weather1") # 'clear'
    assert ret == 1
    # 'store [device]'
    ret,_ = await parser_object.script_command_parser(room_conf, "store button1 state button1_state1")
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store dimmer1 state dimmer1_state1")
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store dimmer1 state_ratio dimmer1_stateratio1")
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store led1 state led1_state1")
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store led1 max_lumen led1_maxlumen1")
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store led1 effective_lumen led1_efflumen1")
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store led1 state_ratio led1_stateratio1")
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store led1 beam_angle led1_beamangle1")
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store heater1 state heater1_state1")
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store heater1 max_power heater1_maxpower1")
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store heater1 effective_power heater1_effpower1")
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store heater1 state_ratio heater1_stateratio1")
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store switch1 state switch1_state1")
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store brightness1 brightness brightness1_bright1")
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store humiditysoil1 humiditysoil humiditysoil1_moisture1")
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store airsensor1 temperature airsensor1_temp1")
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store airsensor1 humidity airsensor1_temp1")
    assert ret == 1
    ret,_ = await parser_object.script_command_parser(room_conf, "store airsensor1 co2 airsensor1_co21")
    assert ret == 1
    ...
    ret, assertions = await parser_object.script_command_parser(room_conf, "assert hum2 >= hum1")
    assert ret == 1
    assert len(assertions) > 0
    for key in assertions.keys(): # key with simtime
        if "Assertion True2" in key:
            assert "hum2 >= hum1" in assertions[key]
    ret, assertions = await parser_object.script_command_parser(room_conf, "assert co22 == co21")
    assert ret == 1
    assert len(assertions) > 0
    for key in assertions.keys(): # key with simtime
        if "Assertion True3" in key:
            assert "co22 == co21" in assertions[key]
    ret, assertions = await parser_object.script_command_parser(room_conf, "assert weather2 != weather1")
    assert ret == 1
    assert len(assertions) > 0
    for key in assertions.keys(): # key with simtime
        if "Assertion True4" in key:
            assert "weather2 != weather1" in assertions[key]
    ret, assertions = await parser_object.script_command_parser(room_conf, "assert bright2 <= bright1")
    assert ret == 1
    assert len(assertions) > 0
    for key in assertions.keys(): # key with simtime
