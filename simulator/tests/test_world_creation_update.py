...
def test_correct_world_creation():
    ...
    world_object = world.World(speed_factor, system_dt, datetime, weather, insulation, temp_out, hum_out, co2_out, temp_in, hum_in, co2_in)
    ...
    assert world_object.ambient_temperature._AmbientTemperature__update_rule_ratio == update_rule_ratio 
    ...
    assert world_object.ambient_temperature._AmbientTemperature__room_insulation == insulation 
    ...
    assert world_object.ambient_humidity._AmbientHumidity__saturation_vapour_pressure_out == p_sat
    assert world_object.ambient_humidity._AmbientHumidity__humidity_in == hum_in 
    assert world_object.ambient_humidity._AmbientHumidity__vapor_pressure_in == vapor_pressure
    assert world_object.ambient_humidity._AmbientHumidity__vapor_pressure_out == vapor_pressure
    assert world_object.ambient_humidity._AmbientHumidity__update_rule_ratio == update_rule_ratio 
...
def test_world_configfile_creation_and_update():
    ...
    date_time = datetime(year=int(dt[0]),month=int(dt[1]),day=int(dt[2]), hour=int(dt[3]), minute=int(dt[4]))
    time_of_day = "sun"
    weather = "clear"
    lux_out = 10752
    brightness = 71.0
    ...
    room_conf, system_dt = tools.configure_system_from_file(config_path, system_dt=system_dt, test_mode=True)
    ...
    assert world_conf.ambient_temperature._AmbientTemperature__room_insulation == insulation
    assert world_conf.ambient_temperature._AmbientTemperature__update_rule_ratio == update_rule_ratio
    assert ir_heater1 in world_conf.ambient_temperature._AmbientTemperature__temp_sources
    assert ir_airsensor1 in world_conf.ambient_temperature._AmbientTemperature__temp_sensors
    ...
    assert world_conf.ambient_humidity._AmbientHumidity__update_rule_ratio == update_rule_ratio
    assert world_conf.ambient_humidity._AmbientHumidity__saturation_vapour_pressure_out == p_sat_out
    assert world_conf.ambient_humidity._AmbientHumidity__saturation_vapour_pressure_in == p_sat_in
    assert world_conf.ambient_humidity._AmbientHumidity__vapor_pressure_out == vapor_pressure_out
    assert world_conf.ambient_humidity._AmbientHumidity__vapor_pressure_in == vapor_pressure_in
    assert ir_airsensor1 in world_conf.ambient_humidity._AmbientHumidity__humidity_sensors
    ...
    assert world_conf.soil_moisture._SoilMoisture__update_rule_ratio == update_rule_ratio
    assert ir_humiditysoil1 in world_conf.soil_moisture._SoilMoisture__humiditysoil_sensors
    ...
    system_brightness1 = world_conf.ambient_light._AmbientLight__light_sensors[world_conf.ambient_light._AmbientLight__light_sensors.index(ir_brightness1)].device
    ...
    # Update World state
    room_conf.update_world(interval=system_dt, gui_mode=False) # second update change the value from one system_dt
    new_temp_in = 24.0
    new_hum_in = 52.51
    new_co2_in = 793
    ...
    assert round(system_brightness1.brightness, 1) == brightness
    # Temperature
    assert world_conf.ambient_temperature._AmbientTemperature__temperature_in == new_temp_in
    assert round(system_airsensor1.temperature, 1) == new_temp_in
    # Humidity
    assert round(world_conf.ambient_humidity._AmbientHumidity__humidity_in, 2) == new_hum_in
    assert world_conf.ambient_humidity._AmbientHumidity__saturation_vapour_pressure_in == new_p_sat_in
    assert world_conf.ambient_humidity._AmbientHumidity__vapor_pressure_in == new_vapor_pressure_in
