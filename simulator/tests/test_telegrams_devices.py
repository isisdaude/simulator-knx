...
def test_detach():
    ...
    assert before.get('group_addresses').get(ga1, None) != after.get('group_addresses').get(ga1, None)
...
def test_dev_info():
    ...
    assert button1.get_dev_info() == {'state': False, 'class_name': 'Button', 'individual_address': '0.0.12'}
    assert dimmer1.get_dev_info() == {'state': False, 'state_ratio': 100, 'class_name': 'Dimmer', 'individual_address': '0.0.13'}
    
    assert led1.get_dev_info() == {'state': False, 'max_lumen': 800, 'effective_lumen': 800.0, 'beam_angle': 180, 'state_ratio': 100, 'class_name': 'LED', 'individual_address': '0.0.1'}
    assert heater1.get_dev_info() == {'state': False, 'update_rule': 1, 'max_power': 400, 'state_ratio': 100, 'effective_power': 400.0, 'class_name': 'Heater', 'individual_address': '0.0.2'}
    assert ac1.get_dev_info() == {'state': False, 'update_rule': -1, 'max_power': 400, 'state_ratio': 100, 'effective_power': 400.0, 'class_name': 'AC', 'individual_address': '0.0.3'}
