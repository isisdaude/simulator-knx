...
def user_command_parser(command: str, room) -> Union[int, None]: # room : Room
    ...
    if command.isspace() or len(command) == 0: # if user pressed enter without any command
        return 1
    if len(command):
...
class ScriptParser():
    ...
    async def script_command_parser(self, room, command: str) -> Tuple[Union[None, int], Dict[str, str]]:
