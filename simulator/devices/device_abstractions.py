...
class Device(ABC):
    ...
    def __init__(self, name: str, individual_addr: IndividualAddress) -> None: #The constructor is also a good place for imposing various checks on attribute values
        ...
        self.name, self.individual_addr = check_device_config(class_name, name, individual_addr)
        self._dev_basic_dict = {"class_name":class_name, "individual_address":self.individual_addr.ia_str} # Prepare dict for CLI 'getinfo' command
    ...
    def send_telegram(self, payload: Payload) -> None:
        ...
        for group_address in self.group_addresses:
            ...
            try:
                ...
                logging.warning(f"[Device.send_telegram()] - Transmission of the telegram from source '{telegram.source}' failed:{exc} with trace \n{trace}.")
