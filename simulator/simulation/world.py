...
class AmbientTemperature:
    ...
    def update(self):
        ...
        if(not self.__temp_sources):
            self.temperature_in += (self.temperature_out - self.temperature_in) * INSULATION_TO_TEMPERATURE_FACTOR[self.__room_insulation] 
        else:
            ...
            for source in self.__temp_sources: # sources of heat or cold
                if source.device.status and source.device.state: # if source enabled
                    if isinstance(source.device, Heater):
                        ...
                        self.temperature_in += source.device.update_rule*self.__update_rule_ratio
                    if isinstance(source.device, AC):
                        source.device.update_rule = - source.device.power/self.total_max_power
                        self.temperature_in += source.device.update_rule*self.__update_rule_ratio # The ac update rule is <0
            # Compute max temp
            ...
            min_temp = 10.0
            self.temperature_in = max(min_temp, min(max_temp, self.temperature_in)) # temperature cannot exceed max temp and be less than min_temp
...
class AmbientLight:
    ...
    def __compute_sensor_brightness(self, brightness_sensor): # Read brightness at a particular sensor
        ...
        for source in self.__light_sources:
            ...
            if isinstance(source.device, system.Window) or (source.device.is_enabled() and source.device.state):
...
class AmbientHumidity:
    ...
    def update(self, temperature):
        ...
        self.humidity += (self.humidity_out - self.humidity) * INSULATION_TO_HUMIDITY_FACTOR[self.__room_insulation] * self.__update_rule_ratio
...
class AmbientCO2:
    ...
    def update(self, temperature, humidity):  ### TODO remove temp et humif not used
        ...
        # Apply humidity factor from outside temp and insulation
        self.co2level += (self.co2_out - self.co2level) * INSULATION_TO_CO2_FACTOR[self.__room_insulation] * self.__update_rule_ratio
        co2_levels = []
    ...
    def update(self, humidity_in):
        ...
        for sensor in self.humiditysoil_sensors:
            if sensor.device.humiditysoil > humidity_in:
                moisture_delta = self.update_rule * self.__update_rule_ratio 
                ...
                moisture_delta = self.update_rule_up * self.__update_rule_ratio 
                if (sensor.device.humiditysoil+moisture_delta) > humidity_in: 
                    sensor.device.humiditysoil = humidity_in
            if sensor.device.humiditysoil != humidity_in:
...
class World:
    ...
    def __init__(self, room_width, room_length, room_height, simulation_speed_factor, system_dt, room_insulation, temp_out, hum_out, co2_out, temp_in, hum_in, co2_in, date_time, weather): #date_time is simply a string keyword from config file at this point
        ...
        self.ambient_humidity = AmbientHumidity(self.temp_out, self.hum_out, hum_in,  self.__room_insulation, self.time.update_rule_ratio)
        self.ambient_co2 = AmbientCO2(self.co2_out, co2_in, self.__room_insulation, self.time.update_rule_ratio)
    ...
    def update(self):
        ...
        humidity_levels = self.ambient_humidity.update(self.ambient_temperature.temperature_in)
        co2_levels = self.ambient_co2.update(self.ambient_temperature.temperature_in, self.ambient_humidity.humidity)
    ...
    def get_info(self, ambient, room, str_mode):
        basic_dict = {"room_insulation":self.__room_insulation, "temperature_out":str(self.temp_out)+" °C", "humidity_out":str(self.hum_out)+" %", "co2_out":str(self.co2_out)+" ppm"}
