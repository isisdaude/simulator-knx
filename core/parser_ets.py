...
def build_manufacturers_list(master_parser, project_files_path):
    ...
    for folder_name in os.listdir(project_files_path):
        if os.path.isdir(project_files_path+folder_name) and folder_name[0:2] == 'M-': #check if the folder has the refid of a manufacturer
...
class Item:
    def __init__(self,  manufacturer_refid, manufacturer_name,
                        firstlevel_section_refid, firstlevel_section_name,
                        secondlevel_section_refid, secondlevel_section_name,
                        item_refid, item_name, item_product_refid, item_hardware2program_refid):
...
def build_devices_list(project_files_path, manufacturers_list):
    ...
    for man in manufacturers_list:
        manufacturer_refid = man.man_refid # frst letter upper case because 'xml' feature and not 'lxml' for manufacturers
        ...
        catalog_parser = BeautifulSoup(catalog_content, 'lxml') # 'lxml' for the catalog, don't ask me why
        ...
        languages = catalog_parser.find(identifier="en-US") #with lower-case first letter, different from the actual file, maybe because of 'lxml' beutiful soup feature
        languages_translation_units = languages.find_all('translationunit') # get all language translation for the devices of this manufacturer that are present in the project
        catalog = catalog_parser.find('catalog') # begining pf catalog tree of devices from this maufacturer in the current project
        # loop on all 1st level of catalogsection (in case there is more than one subsection)
        for firstlevel_section in catalog.findChildren('catalogsection', recursive = False):
            firstlevel_section_refid = firstlevel_section['id']
            name = find_english_name(firstlevel_section_refid, languages_translation_units)
            firstlevel_section_name = name if name else firstlevel_section['name'] # if no translation found
            # loop on all sub catalogsection for the current 1st level catalog section
            for secondlevel_section in firstlevel_section.findChildren('catalogsection', recursive = False):
                secondlevel_section_refid = secondlevel_section['id']
                name = find_english_name(secondlevel_section_refid, languages_translation_units)
                secondlevel_section_name = name if name else secondlevel_section['name'] # if no translation found
                # loop on all item defined under this 2-level catalogsections
                for item in secondlevel_section.findChildren('catalogitem', recursive = False):
                    item_refid = item['id']
                    ...
                    item_obj = Item(manufacturer_refid, manufacturer_name,
                                    firstlevel_section_refid, firstlevel_section_name,
                                    secondlevel_section_refid, secondlevel_section_name,
                                    item_refid, item_name, item_product_refid, item_hardware2program_refid)
...
def main(): #TODO: consider device number in the case of multiple instances of the same deviiec in a project
    ...
    project_files_path = "./catalogproject_sensors/"
