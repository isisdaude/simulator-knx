...
class DeviceWidget(object):
    def __init__(self, x, y, batch, img_file_ON, img_file_OFF, group, device_type, device_name, device_number):
        self.name = device_name
        self.label_name = self.name+' '+device_number
...
class AvailableDevices(object): # library of devices availables, presented on the left side on the GUI
    def __init__(self, batch, group):
        ...
        self.devices = []
        self.lightbulb = DeviceWidget(100, 750, batch, 'png_simulator/lightbulb_ON.png', 'png_simulator/lightbulb_OFF.png', group, "actuator", "LED", '')
        ...
        self.brightness_sensor = DeviceWidget(170, 750, batch, 'png_simulator/brightness_sensor.png', 'png_simulator/brightness_sensor.png', group, "sensor", "Bright", '')
        self.devices.append(self.brightness_sensor)
...
class GUIWindow(pyglet.window.Window):
    ...
    def __init__(self, room=None):
        ...
        self.devicelist_pos = (2*(self.width//100), 50*(self.height//100))
    ...
    # def initialize_system(self, room):
                ...
                if not hasattr(self, 'moving_device'):
                    for device in self.available_devices.devices:
                        ...
                        if device.hit_test(x, y):
                            ...
                            self.moving_device = DeviceWidget(x, y, self.batch, device.file_ON, device.file_OFF, group=self.foreground, device_type=device_type, device_name=device.name, device_number=str(similar_dev_counter))
                            return
                    for room_device in self.room_devices:
