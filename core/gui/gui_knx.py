...
OFFSET_LIST = 20
OFFSET_LABEL_DEVICE = 10
OFFSET_LABEL_BUTTON = 10
OFFSET_SENSOR_LEVELS = 100
...
class DeviceWidget(object):
    def __init__(self, x, y, batch, img_file_ON, img_file_OFF, group, device_type, device_class, device_number):
        ...
        self.sprite = pyglet.sprite.Sprite(self.img_OFF, x=self.pos_x, y=self.pos_y, batch=self.batch, group=self.group) #x,y is bottom left of image
        self.label = pyglet.text.Label(self.label_name,
                                    font_name='Times New Roman', font_size=10,
                                    x=(self.pos_x+self.width//2), y=(self.pos_y-OFFSET_LABEL_DEVICE),
    ...
    def update_position(self, new_x, new_y, loc_x=0, loc_y=0, update_loc=False):
        ...
        self.sprite.update(x=self.pos_x, y=self.pos_y)
        self.label.update(x=(self.pos_x+self.width//2), y=(self.pos_y-OFFSET_LABEL_DEVICE))
        if update_loc:
...
class RoomWidget(object):
    def __init__(self, width, length, batch, group):
        ...
        self.pos_x_shape = WIN_WIDTH - width - WIN_BORDER - 2*ROOM_BORDER
        self.pos_y_shape = WIN_BORDER
        # Cordinates to represent the actual room dimensions (border is a margin for devices on boundaries)
        self.pos_x = WIN_WIDTH - width - WIN_BORDER - ROOM_BORDER
        self.pos_y = WIN_BORDER + ROOM_BORDER
        # Actual dimensions of the room, not the rectangle shape
        self.width = width
        self.length = length
        self.batch = batch
        self.shape = pyglet.shapes.BorderedRectangle(self.pos_x_shape, self.pos_y_shape, width+2*ROOM_BORDER, length+2*ROOM_BORDER, border=ROOM_BORDER,
...
class AvailableDevices(object): # library of devices availables, presented on the left side on the GUI
    def __init__(self, batch, group):
        ...
        self.devices = []
        self.led = DeviceWidget(100, 800, batch, DEVICE_LED_ON_PATH, DEVICE_LED_OFF_PATH, group, "actuator", "LED", '')
        self.devices.append(self.led)
        self.switch = DeviceWidget(30, 800, batch, DEVICE_SWITCH_ON_PATH, DEVICE_SWITCH_OFF_PATH, group, "functional_module", "Switch", '')
        self.devices.append(self.switch)
        self.brightness = DeviceWidget(170, 800, batch, DEVICE_BRIGHT_SENSOR_PATH, DEVICE_BRIGHT_SENSOR_PATH, group, "sensor", "Brightness", '')
        self.devices.append(self.brightness)
        self.thermometer = DeviceWidget(280, 800, batch, DEVICE_THERMO_COLD_PATH, DEVICE_THERMO_HOT_PATH, group, "sensor", "Thermometer", '')
...
class GUIWindow(pyglet.window.Window):
    ...
    def __init__(self, config_path, default_config_path, rooms=None):
        ...
        self.devicelist_pos = (2*(self.width//100), 70*(self.length//100))
        self.brightness_label_pos = (2*(self.width//100), 30*(self.length//100))
        self.brightness_level_pos = self.brightness_label_pos[0], self.brightness_label_pos[1]-20
        self.temperature_label_pos = (2*(self.width//100), 15*(self.length//100))
        ...
        self.button_pause_pos = (55*(self.width//100), 92*(self.length//100))
        self.button_stop_pos = (61*(self.width//100), 92*(self.length//100))
        self.button_reload_pos = (67*(self.width//100), 92*(self.length//100))
        self.button_default_pos = (73*(self.width//100), 92*(self.length//100))
    ...
    def initialize_system(self):
        ...
        for in_room_device in self.room.devices:
            ...
            if isinstance(device, Brightness):
                ...
                pos_x, pos_y = system_loc_to_gui_pos(in_room_device.location.x, in_room_device.location.y, self.room_width_ratio, self.room_length_ratio, self.room_widget.pos_x, self.room_widget.pos_y)
                print(f"{device.name} is at  {pos_x},{pos_y}")
    ...
    def display_devices_list(self):
        ...
        for room_device in self.room_devices:
            ...
            room_dev_y = self.deviceslist_label.y - room_devices_counter*OFFSET_LIST # display device name below main label
    ...
    def display_temperature_labels(self):
        ...
        bright_x = self.temperature_label.x
        bright_y = self.temperature_label.y - OFFSET_LIST
        # Display the current room devices name in list
        for room_device in self.room_devices:
            if 'thermometer' in room_device.label.text.lower():
                room_bright_x = bright_x + room_temperature_counter*OFFSET_SENSOR_LEVELS
    ...
    def display_brightness_labels(self):
        ...
        bright_y = self.brightness_label.y - OFFSET_LIST
        # Display the current room devices name in list
        for room_device in self.room_devices:
    ...
    def display_brightness_level(self, bright_name, brightness):
        ...
        for room_brightness_label in self.room_brightness_labels:
            if bright_name[-1] == room_brightness_label.text[-1]: #check only the digit
                ...
                bright_level_y = room_brightness_label.y - OFFSET_LIST
            ...
            try:
                if room_device.sprite_state != room_device.in_room_device.device.state: #sprite represetation is not the same as real state
                    ...
                    if room_device.sprite_state: # device turned ON
                        room_device.sprite = pyglet.sprite.Sprite(room_device.img_ON, x=room_device.pos_x, y=room_device.pos_y, batch=self.batch, group=self.foreground)
                    else: # device turned OFF
                        room_device.sprite = pyglet.sprite.Sprite(room_device.img_OFF, x=room_device.pos_x, y=room_device.pos_y, batch=self.batch, group=self.foreground)
    ...
    def add_device(self):
        ...
        loc_x, loc_y = gui_pos_to_system_loc(self.moving_device.pos_x, self.moving_device.pos_y,
                                                ...
                                                self.room_widget.pos_x, self.room_widget.pos_y)
        ...
        if button == pyglet.window.mouse.LEFT:
            ...
            if hasattr(self, 'moving_device'):
                ...
                if self.room_widget.hit_test(x, y):
                    pos_x, pos_y = x - (self.moving_device.width//2), y - (self.moving_device.length//2)
                    loc_x, loc_y = gui_pos_to_system_loc(pos_x, pos_y, self.room_width_ratio, self.room_width_ratio, self.room_widget.pos_x, self.room_widget.pos_y)
    ...
    def replace_moving_device_in_room(self, x, y):
        from system import Location
        x_min = self.room_widget.pos_x + self.moving_device.width//2
        x_max = self.room_widget.pos_x + self.room_widget.width - self.moving_device.width//2
        y_min = self.room_widget.pos_y + self.moving_device.length//2
        y_max = self.room_widget.pos_y + self.room_widget.length - self.moving_device.length//2
